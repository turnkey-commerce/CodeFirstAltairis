	...
	GetControllerScaffolders = {
		...
		$result = $allControllerScaffolders | %{
			global:MvcScaffoldingHashTableToPsObject @{
				...
				Execute = { 
					param($container, $controllerName, $modelType, $dataContextType, $viewsScaffolder, $viewOptions)
					$scriptToExecute = @"
						...
						`$possibleArgs = @{
							...
							Force = $(if($viewOptions.OverwriteViews) { "`$true" } else { "`$false" });
						}
						...
						`$acceptedParameterNames | ?{ `$possibleArgs[`$_] } | %{ `$actualArgs.Add(`$_, `$possibleArgs[`$_]) }
